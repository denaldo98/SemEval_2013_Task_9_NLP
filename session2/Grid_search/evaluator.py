...
def print_statistics(gold,predicted, outfile) :

    with open('devel-CRF.stats', 'a') as modelstats:
        print(outfile + ' :', file=modelstats)
        print(row("")+"  tp\t  fp\t  fn\t#pred\t#exp\tP\tR\tF1", file=modelstats)
        print("------------------------------------------------------------------------------", file=modelstats)
        ...
        for kind in sorted(gold) :
            ...
            print(row(kind)+"{:>4}\t{:>4}\t{:>4}\t{:>4}\t{:>4}\t{:2.1%}\t{:2.1%}\t{:2.1%}".format(tp,fp,fn,npred,nexp, P, R, F1), file=modelstats)
            (nk,sP,sR,sF1) = (nk+1, sP+P, sR+R, sF1+F1)

        (sP, sR, sF1) = (sP/nk, sR/nk, sF1/nk)
        print("------------------------------------------------------------------------------", file=modelstats)
        print(row("M.avg")+"-\t-\t-\t-\t-\t{:2.1%}\t{:2.1%}\t{:2.1%}".format(sP, sR, sF1), file=modelstats)

        print("------------------------------------------------------------------------------", file=modelstats)
        (tp,fp,fn,npred,nexp,P,R,F1) = statistics(gold, predicted, "CLASS")
        print(row("m.avg")+"{:>4}\t{:>4}\t{:>4}\t{:>4}\t{:>4}\t{:2.1%}\t{:2.1%}\t{:2.1%}".format(tp,fp,fn,npred,nexp, P, R, F1), file=modelstats)                        
        (tp,fp,fn,npred,nexp,P,R,F1) = statistics(gold, predicted, "NOCLASS")
        print(row("m.avg(no class)")+"{:>4}\t{:>4}\t{:>4}\t{:>4}\t{:>4}\t{:2.1%}\t{:2.1%}\t{:2.1%}".format(tp,fp,fn,npred,nexp, P, R, F1), file=modelstats) 
        print('\n', file=modelstats)              
...
def evaluate(task, golddir, outdir, outfile):
    ...
    predicted = load_predicted(task, outdir + '/' + outfile)

    # compare both sets and compute statistics
    print_statistics(gold,predicted, outfile)
...
if __name__ == "__main__":
    ...
    # folder name CRFs: ./devel-CRFs
    outdir = sys.argv[2]
    files = os.listdir(outdir)

    for file in files:
        if os.path.isfile(os.path.join(outdir, file)):
            evaluate('NER', golddir, outdir, file)